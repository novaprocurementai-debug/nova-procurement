  ...
  async fetch(request, env) {
    ...
    if (request.method === "POST" && url.pathname === "/api/search") {
      ...
      try {
        ...
        const searchQuery = `
${requestText}
manufacturer factory wholesale OEM ODM bulk supplier
MOQ quotation unit price customization shipping
`.replace(/\s+/g, " ").trim();
        ...
        const results = data.results
          ...
          .map(x => {
            ...
            const shippingMentioned =
              /\b(shipping|freight|delivery|FOB|CIF|DDP|EXW)\b/i
              .test(text);
...
You are NOVA, an evidence-first
global procurement AI.

Your most important rule:

NEVER invent, infer, assume,
or imply a fact that the supplier
...
Analyze ONLY the information
explicitly provided.

For every field below use:
...
Do NOT treat quantity as MOQ.

Do NOT assume customization
is included.

Do NOT assume shipping destination.

Do NOT call a price competitive
unless there is evidence for comparison.
            ...
            MODEL,
            {
              messages: [

                {
                  ...
                  content:
                    "You are NOVA, an evidence-first procurement agent. Never invent facts or numbers."
                },
